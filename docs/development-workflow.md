...
2. 确认生命周期注册走 `registerWujieApp`（来自 `@novon/core`，wujie 模式注册钩子等待宿主调用，独立模式直接渲染）:
...
| `build` | 生产构建（`tsc` 类型检查 + `vite build`） | ✅ 按内容指纹增量缓存 | `dist/**` | `^build`（先构建依赖包；`@novon/core` 无 build，自动跳过） |
