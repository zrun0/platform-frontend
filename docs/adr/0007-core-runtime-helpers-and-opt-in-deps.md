...
**Accepted**（2026-08-29）部分调整 [0003](./0003-why-shared-common-source-consumption.md) 的内容边界约束与 [0006](./0006-rename-portal-and-novon-core.md) 的「一切包依赖它」理由；Decision #3（Node 侧配置不 import core 源码）已被 [0009](./0009-shared-vite-config-factory-in-core.md) 取代（2026-09-18），其余决策不变
...
- **新建 `@novon/wujie` 运行时包** ✅ core 边界不动 / ❌ 当前仅一处共享，第二底座包是过早分层；需要时再拆不迟
...
- [0006 - 主基座更名 portal、核心包更名 @novon/core](./0006-rename-portal-and-novon-core.md)（命名与分层不变，依赖策略改为按需）
