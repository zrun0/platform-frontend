# 0006 - 主基座更名 portal、核心包更名 @novon/core
...
- 核心共享包：`packages/shared-common` → `packages/core`，包名加公司 scope：`@novon/core`（目录名保持 `packages/core`）
- apps 不加 scope（永不发布，保持短名利于 `pnpm --filter`）；`packages/*` 统一 `@novon/<name>`

Why：

- **portal**：表达「统一门户入口」的定位，消除语义冗余，与子应用短名风格一致
- **core + scope**：多包分层结构中 `core` 语义为「底座层（foundation layer）」（同 `@angular/core`、`@nestjs/core` 用法）：一切包依赖它、它零依赖；`@novon/` scope 防同名冲突、为内部 registry 发布预留
...
Positive：命名体系可扩展（`@novon/ui`、`@novon/request`…）；名实相符（`core` 的分层语义由 README 规则锚定）。
